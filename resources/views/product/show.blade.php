        ...
        <div class="flex flex-col md:flex-row gap-8">
            ...
            <div class="flex-1">
                ...
                <div class="main-image-container relative aspect-square bg-gray-50 rounded-lg overflow-hidden border border-gray-100 mb-4">
                     @php
                        // Construct Image URL
                        // Legacy logic mainly used: 'ims/internal/' . img2 for main image
                        $mainImgUrl = asset('ims/internal/' . ($product->img2 ?? 'placeholder.jpg'));
                    @endphp
                ...
                <div class="flex space-x-2 overflow-x-auto pb-2">
                    ...
                    @for ($i = 2; $i <= 5; $i++)
                        ...
                        @endphp
                        @if ($imgVal)
                             @php $thumbUrl = asset('ims/internal/' . $imgVal); @endphp
        ...
        @if(isset($similarProducts) && count($similarProducts) > 0)
            <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
                @foreach($similarProducts as $sim)
                    @php 
                         $simImgUrl = asset('ims/internal/' . ($sim->img2 ?? 'placeholder.jpg'));
                    @endphp
